...
class UpdateData():
    ...
    def UpdateTransactions(self):
        ...
        url = 'https://fantasysports.yahooapis.com/fantasy/v2/league/' + leaguevar +  '/transactions'
    ...
    def UpdateLeague(self):
        # LEAGUE OVERVIEW
        yahoo_api._login()
        url = 'https://fantasysports.yahooapis.com/fantasy/v2/league/' + leaguevar +  '/'
        response = oauth.session.get(url, params={'format': 'json'})
        r = response.json()
    ...
    def UpdateLeagueStandings(self):
        # STANDINGS
        yahoo_api._login()
        url = 'https://fantasysports.yahooapis.com/fantasy/v2/league/' + leaguevar +  '/standings'
        response = oauth.session.get(url, params={'format': 'json'})
        r = response.json()
    ...
    def UpdateScoreboards(self):
        ...
        while week < num_weeks+1: #assumes 16 week-schedule
            url = 'https://fantasysports.yahooapis.com/fantasy/v2/league/' + leaguevar +  '/scoreboard;week='+str(week)
            response = oauth.session.get(url, params={'format': 'json'})
            r = response.json()
    ...
    def UpdateRosters(self):
        ...
        for week in range(1, num_weeks+1): #assumes 16-week schedule
            ...
            for team in range(1, num_teams+1): #assumes 12-team league
                url = 'https://fantasysports.yahooapis.com/fantasy/v2/team/' + leaguevar +  '.t.'+str(team)+'/roster;week='+str(week)
                response = oauth.session.get(url, params={'format': 'json'})
                r = response.json()
                #Looking for directory, creating new one if it doesn't exist
                if os.path.isdir('./rosters/week_'+str(week)+'/') == False:
                    os.mkdir('./rosters/week_'+str(week)+'/')
